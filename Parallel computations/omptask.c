...
//printf("rank = %d: y ended\n", Comm_rank);
						...
						if (only_z)
						{
							...
							else
							{
								...
								{
									int partner_rank = FindBlockRank(x, y, z_div - 1, x_div, y_div);
									double Gij1 = 2*Grid[i][j][1].now - Grid[i][j][1].before + tau*tau*Grid[i][j][1].after/(h*h);

									MPI_Send(&Gij1, 1, MPI_DOUBLE, partner_rank, 3, MPI_COMM_WORLD);
									MPI_Recv(&Grid[i][j][0].after, 1, MPI_DOUBLE, partner_rank, 4, MPI_COMM_WORLD, &status);
								}
								else
								{
									double Gij1 = 2*Grid[i][j][1].now - Grid[i][j][1].before + tau*tau*Grid[i][j][1].after/(h*h);
									double Gijk_1 = 2*Grid[i][j][k-1].now - Grid[i][j][k-1].before + tau*tau*Grid[i][j][k-1].after/(h*h);
				//if ((i == 2) && (j == 2) && (k == 4)) printf("rank = %d: step = %d, Gij1 = %f, Gijk_1 = %f\n", Comm_rank, step, Gij1, Gijk_1);
									Grid[i][j][k].after = (Gij1 + Gijk_1)/2;
									Grid[i][j][0].after = Grid[i][j][k].after;///zhopa
								}
							}
...
//printf("rank = %d: z ended\n", Comm_rank);
		...
		for (int i=0; i<x_size; i++)
		{
			for (int j=0; j<y_size; j++)
			{
				...
				{
					Grid[i][j][k].after = 2*Grid[i][j][k].now - Grid[i][j][k].before + tau*tau*Grid[i][j][k].after/(h*h);
				}
			}
		}
